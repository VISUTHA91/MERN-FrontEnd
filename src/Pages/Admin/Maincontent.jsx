...
import { Link } from 'react-router-dom';
import { fetchDashboardStats } from '../../api/apiServices';
import { RadialBarChart, RadialBar, Legend, Tooltip, Text } from 'recharts';
    ...
    <div className="">
      <div className="flex-grow p-6">
        <div className="bg-white rounded-xl lg:h-32 h-44 relative">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 m-8 absolute ">
            <div className="bg-blue-500 text-white p-4 rounded-lg shadow-lg lg:w-44 lg:h-24">
              <div className="text-center lg:text-xl">Total Sales</div>
              <div className="text-center lg:text-2xl font-bold">{chartData.reduce((acc, item) => acc + item.value, 0)}</div>
            </div>

            <div className="bg-green-500 text-white p-4 rounded-lg shadow-lg lg:w-44 lg:h-24">
              <div className="text-center lg:text-xl">Products</div>
              <div className="text-center lg:text-2xl font-bold">{chartData.find(item => item.name === "Products")?.value}</div>
              <Link to="/Admin/AdminProductlist" className="block mt-4 text-center text-sm underline hover:text-gray-200">
                View Products
              </Link>
            </div>

            <div className="bg-red-500 text-white p-4 rounded-lg shadow-lg lg:w-44 lg:h-24">
              <div className="text-center lg:text-xl">Orders</div>
              <div className="text-center lg:text-2xl font-bold">{chartData.find(item => item.name === "Orders")?.value}</div>
              <Link to="/Admin/AdminOrders" className="block mt-4 text-center text-sm underline hover:text-gray-200">
                View Orders
              </Link>
            </div>

            <div className="bg-teal-500 text-white p-4 rounded-lg shadow-lg lg:w-44 lg:h-24">
              <div className="text-center lg:text-xl">Users</div>
              <div className="text-center lg:text-2xl font-bold">784521</div>
              <Link to="/Admin/Userlist" className="block mt-4 text-center text-sm underline hover:text-gray-200">
                View Users
              </Link>
            </div>

            <div className="bg-yellow-500 text-white p-4 rounded-lg shadow-lg lg:w-44 lg:h-24">
              <div className="text-center text-xl">Out of Stock</div>
              <div className="text-center text-2xl font-bold">{chartData.find(item => item.name === "Out of Stock")?.value}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 mt-8 mb-10">
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {chartData.map((item, index) => (
            <div key={index} className="flex flex-col items-center">
              <RadialBarChart width={200} height={200} innerRadius="80%" outerRadius="90%" barSize={15} data={[item]}>
                <RadialBar minAngle={15} clockWise dataKey="value" />
                <Text
                  x={100}
                  y={100}
