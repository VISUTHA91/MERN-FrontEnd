  ...
  return (
    ...
    <div className="flex flex-col lg:flex-row p-4 sm:p-6 lg:p-12 ">
                  ...
                  <div key={index} className="w-16 h-16 sm:w-20 sm:h-20">
                    <img
                      ...
                      className={` border-black rounded-xl cursor-pointer max-sm:flex-1 border${
                        activeImage === imageURL ? ' ring-1 ring-purple-900' : ' border border-1'
                      }`} onClick={() => handleMouseEnterProduct(imageURL)}
                    />
                  </div>
                ))}
      ...
      <div className='flex flex-wrap gap-2 mt-2'>
          <input type='button' className={`border border-black  rounded-full  h-8 w-8 bg-red-500 ${ 
            selectedColor === 'red' ? 'border-black border-2' : 'border-1'
            }`}onClick={() => handleColorChange('red')}/>
          <input type="button"  name="green" className={`border border-black rounded-full h-8 w-8 bg-green-500 ${
              selectedColor === 'green' ? 'border-black border-2' : 'border-1'
            }`} onClick={() => handleColorChange('green')}/>
          <input type="button" name="blue" className={`border border-black rounded-full h-8 w-8 bg-blue-500 ${
              selectedColor === 'blue' ? 'border-black border-2' : 'border-1'
            }`} onClick={() => handleColorChange('blue')} />
          <input type="button"  name="white" className={`border border-black rounded-full h-8 w-8 bg-white ${
              selectedColor === 'white' ? 'border-black border-2' : 'border-1'
            }`} onClick={() => handleColorChange('white')}/>
          <input type="button" name="pink" className={`border border-black rounded-full h-8 w-8 bg-pink-500 ${
              selectedColor === 'pink' ? 'border-black border-2' : 'border-1'
            }`} onClick={() => handleColorChange('pink')}/>
          <input type="button" name="brown" className={`border border-black rounded-full h-8 w-8 bg-rose-800 ${
              selectedColor === 'brown' ? 'border-black border-2' : 'border-1'
            }`} onClick={() => handleColorChange('brown')}/>
          <input type="button"  name="black"className={`border border-black rounded-full h-8 w-8 bg-black ${
              selectedColor === 'black' ? 'border-black border-2' : 'border-1'
            }`} onClick={() => handleColorChange('black')}/>
          <input type="button" name="yellow"className={`border border-black rounded-full h-8 w-8 bg-yellow-500 ${
      ...
      <Tooltip title="Garment Measurement : Chest - 37.0in" placement="top">
          <input type="button" name="s" value="S"className={`border border-black rounded-full h-8 w-8 ${
              selectedSize === 's' ? 'border-black border-2' : 'border-1'
            }`} onClick={() => handleSizeChange('s')}/> </Tooltip>
                  <Tooltip title="Garment Measurement : Chest - 39.0in" placement="top">
               <input type="button" name="s" value="M"className={`border border-black rounded-full h-8 w-8 ${
              selectedSize === 'm' ? 'border-black border-2' : 'border-1'
            }`} onClick={() => handleSizeChange('m')}/></Tooltip>
                  <Tooltip title="Garment Measurement : Chest - 43.0in" placement="top">

               <input type="button" name="s" value="L"className={`border border-black rounded-full h-8 w-8 ${
              selectedSize === 'l' ? 'border-black border-2' : 'border-1'
            }`} onClick={() => handleSizeChange('l')}/></Tooltip>
                  <Tooltip title="Garment Measurement : Chest - 41.0in" placement="top">
               <input type="button" name="s" value="XL"className={`border border-black rounded-full h-8 w-8 ${
              selectedSize === 'xl' ? 'border-black border-2' : 'border-1'
